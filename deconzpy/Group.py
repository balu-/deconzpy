...
class Group(DeconzBaseElement):
    ...
    def actionOn(self):
        r = requests.put(self.getUrlRoot() + "/" + self.getId() + "/action", json={"on": True})
        # print(r.status_code)
        # print(r.text)

    def actionOff(self):
        r = requests.put(self.getUrlRoot() + "/" + self.getId() + "/action", json={"on": False})
    ...
    def println(self):
        print(self.getId() + " " + self.getName() + " - S:" + str(len(self.getScenes())))
